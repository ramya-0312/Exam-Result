...
export class AdminAnalyticsComponent implements OnInit {
  ...
  fetchAnalyticsData(): void {
    this.http.get<any[]>('http://localhost:8080/api/analytics/all-semesters').subscribe({
      next: (data) => {
  ...
  fetchStudentInfo(): void {
    ...
    if (storedReg && storedDob) {
      ...
      this.http.get<any>(`http://localhost:8080/api/student/info?regNo=${storedReg}&dob=${storedDob}`)
        .subscribe({
          next: (res) => {
